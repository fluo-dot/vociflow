...
export default function CourseDetail({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const { courses, decks, loading, addDeckToCourse, removeDeckFromCourse, exportSingleCourse } = useDataStore();
  ...
  if (!course) return <div className="p-20 text-center font-bold text-destructive">Kurs nicht gefunden</div>;

  const courseDecks = decks.filter(d => course.deckIds?.includes(d.id));
  const availableDecks = decks.filter(d => !course.deckIds?.includes(d.id));
      ...
      <main className="container mx-auto px-4 py-8 max-w-5xl space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 bg-white dark:bg-card p-10 rounded-[3rem] shadow-sm border border-slate-100 dark:border-border">
          <div className="space-y-4">
            <div className="flex items-center gap-3 text-accent font-bold text-sm bg-accent/10 w-fit px-4 py-1.5 rounded-full uppercase tracking-widest">
              <GraduationCap className="h-5 w-5" /> KURS
            </div>
            <h1 className="font-headline text-5xl font-bold tracking-tight text-slate-900 dark:text-slate-100">{course.title}</h1>
            <p className="text-slate-500 dark:text-slate-400 text-xl leading-relaxed max-w-2xl">{course.description}</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Dialog>
              ...
              <DialogContent className="rounded-[2rem]">
                <DialogHeader>
                  <DialogTitle>Kurs exportieren</DialogTitle>
                  <DialogDescription>
                    Dieser Export enthält den Kurs "{course.title}" sowie alle {courseDecks.length} darin enthaltenen Lernsets.
                  </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                  <Button onClick={() => exportSingleCourse(id)} className="w-full h-12 rounded-xl font-bold gap-2">
                    <Download className="h-4 w-4" /> Kurs-Paket herunterladen
                  </Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
            ...
            <Dialog open={addModalOpen} onOpenChange={setAddModalOpen}>
              ...
              <DialogContent className="max-w-md rounded-[2rem]">
                <DialogHeader>
                  <DialogTitle>Hefte zum Kurs hinzufügen</DialogTitle>
                  <DialogDescription>
                  ...
                  {availableDecks.length === 0 ? (
                    ...
                    availableDecks.map(deck => (
                      <div key={deck.id} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-border group hover:border-primary/30 transition-all">
                        ...
                        <Button size="sm" variant="ghost" className="text-primary font-bold hover:bg-primary/10 rounded-xl" onClick={() => addDeckToCourse(id, deck.id)}>
                          Hinzufügen
                        </Button>
                      </div>
                    ))
        ...
        <div className="space-y-6">
          <h2 className="font-headline text-3xl font-bold flex items-center gap-4 px-2">
            ...
            In diesem Kurs ({courseDecks.length})
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Card key={deck.id} className="group border-none shadow-sm hover:shadow-lg transition-all rounded-[2.5rem] bg-white dark:bg-card overflow-hidden">
                <CardHeader className="pb-4">
                  <div className="flex justify-between items-start">
                    ...
                    <Button variant="ghost" size="icon" className="h-9 w-9 text-slate-300 hover:text-destructive transition-colors" onClick={() => removeDeckFromCourse(id, deck.id)}>
            ...
            {courseDecks.length === 0 && (
              <div className="md:col-span-2 text-center py-24 bg-slate-50/50 dark:bg-slate-900/10 rounded-[3rem] border-4 border-dashed border-slate-100 dark:border-border">
                ...
                <p className="text-slate-400 text-lg font-medium mb-6">Noch keine Hefte in diesem Kurs.</p>
