...
import { useDataStore, Course, Deck } from "@/lib/db";
...
function ShareContent() {
  ...
  const { importCourse } = useDataStore();

  const [sharedData, setSharedData] = useState<{ course: Course; decks: Deck[] } | null>(null);
  ...
  useEffect(() => {
    ...
    // wenn jemand einen alten Link nutzt.
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      await importCourse(sharedData.course, sharedData.decks);
      setSuccess(true);
  ...
  if (loading) {
    return (
      <div className="flex flex-col min-h-screen items-center justify-center">
        ...
        <p className="mt-4 text-slate-500 font-medium">Lade Kursinformationen...</p>
      </div>
    );
  }
        ...
        {success ? (
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="text-center space-y-4">
              ...
              <p className="text-slate-500 text-lg">Kurs importieren.</p>
            </div>

            <Card className="rounded-[2.5rem] border-none shadow-2xl overflow-hidden bg-white">
              <CardHeader className="p-10 pb-6 border-b bg-slate-50/50">
                <CardTitle className="text-3xl font-headline">{sharedData.course.title}</CardTitle>
                <CardDescription className="text-lg">{sharedData.course.description}</CardDescription>
              </CardHeader>
