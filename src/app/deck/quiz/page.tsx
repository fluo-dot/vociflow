...
import { ArrowLeft, CheckCircle2, XCircle, RefreshCw, Trophy, ChevronRight, Info, ArrowLeftRight, Loader2 } from "lucide-react";
...
function QuizModeContent() {
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    const correct = userInput.trim().toLowerCase() === answerText.trim().toLowerCase();
  ...
  if (!deck || deck.cards.length === 0) return <div className="p-20 text-center text-destructive font-bold">Keine Vokabeln vorhanden.</div>;
      ...
      <main className="container mx-auto px-4 py-20 max-w-2xl text-center">
        <div className="bg-white dark:bg-card p-12 rounded-[3rem] shadow-sm space-y-8 animate-in zoom-in duration-300">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href={`/deck?id=${id}`} className="flex-1">
  ...
  return (
    <main className="container mx-auto px-4 py-10 max-w-3xl space-y-12">
      <div className="flex items-center justify-between">
        <Link href={`/deck?id=${id}`} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors font-bold uppercase tracking-wider">
      ...
      <div className="flex items-center gap-2 text-muted-foreground justify-center text-sm font-medium">
        ...
        <span>Ignoriert Groß-/Kleinschreibung. Nutze Enter zum Bestätigen/Weitergehen.</span>
      </div>
    </main>
  );
}
...
export default function QuizMode() {
  return (
    <div className="flex flex-col min-h-screen bg-background">
      <Navbar />
      <Suspense fallback={<div className="p-20 text-center"><Loader2 className="h-8 w-8 animate-spin mx-auto text-primary" /></div>}>
        <QuizModeContent />
      </Suspense>
    </div>
  );
